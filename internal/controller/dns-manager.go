...
func handleDnsEntries[T v1alpha1.DomainEntity](ctx context.Context, c *Controller, dom T, ownerId, subResourceName string, subResourceNamespace string) (err error) {
	...
	aRelevantDNSNameHashes, err := checkRelevantDNSEntries(ctx, list.Items, relevantDNSInfo, dom.GetMetadata().Generation, c)
	if err != nil {
		return err
	}
...
func checkRelevantDNSEntries(ctx context.Context, dnsEntries []dnsv1alpha1.DNSEntry, relevantDNSInfo map[string]*dnsInfo, generation int64, c *Controller) (aRelevantDNSNameHashes []string, err error) {
	...
	for _, entry := range dnsEntries {
		if info, ok := relevantDNSInfo[entry.Spec.DNSName]; ok {
			...
			if entry.Annotations[AnnotationResourceHash] != hash {
				...
				if err != nil {
					return nil, fmt.Errorf("failed to update dns entry %s.%s: %w", entry.Namespace, entry.Name, err)
				}
			}
	...
	return aRelevantDNSNameHashes, nil
}
