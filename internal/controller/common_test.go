...
var gvrKindMap map[string]string = map[string]string{
	...
	"subscriptionproviders.sme.sap.com/v1alpha1":  "SubscriptionProvider",
	"subscriptions.sme.sap.com/v1alpha1":          "Subscription",
}
...
var removeStatusTimestampHandler k8stesting.ReactionFunc = func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
	...
	if update.GetSubresource() == "status" {
		...
		case *v1alpha1.ClusterDomain:
			cro.Status.Conditions = adjustConditions(cro.Status.Conditions)
		case *v1alpha1.SubscriptionProvider:
			cro.Status.Conditions = adjustConditions(cro.Status.Conditions)
...
func reconcileTestItem(ctx context.Context, t *testing.T, item QueueItem, data TestData) (err error) {
	...
	t.Run(strings.Join(append([]string{data.description}, data.backlogItems...), " "), func(t *testing.T) {
		...
		case ResourceSubscriptionProvider:
			requeue, err = c.reconcileSubscriptionProvider(ctx, item, data.attempts)
...
func addInitialObjectToStore(resource []byte, c *Controller) error {
	...
	case *v1alpha1.CAPApplication, *v1alpha1.CAPApplicationVersion, *v1alpha1.CAPTenant, *v1alpha1.CAPTenantOperation, *v1alpha1.Domain, *v1alpha1.ClusterDomain, *v1alpha1.SubscriptionProvider, *v1alpha1.Subscription:
		...
		case *v1alpha1.SubscriptionProvider:
			err = c.crdInformerFactory.Sme().V1alpha1().SubscriptionProviders().Informer().GetIndexer().Add(obj)
		case *v1alpha1.Subscription:
			err = c.crdInformerFactory.Sme().V1alpha1().Subscriptions().Informer().GetIndexer().Add(obj)
		}
