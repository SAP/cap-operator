...
func checkWorkloadNameLength(cavObjNew *ResponseCav, workload *v1alpha1.WorkloadDetails) validateResource {
	...
	// Tenant job length should not exceed 63 characters considering the generated pod name (final pod name => caName-provider-workloadName-q4m9c-c4d9c)
	// For consumer tenant, the pod name will always be less than the provider tenant pod name, so we only check for provider tenant
	if workload.JobDefinition != nil && (workload.JobDefinition.Type == v1alpha1.JobTenantOperation || workload.JobDefinition.Type == v1alpha1.JobCustomTenantOperation) {
		// Note: The suffix is a dummy value to mimic the generated pod name, actual suffix will be generated by the controller
		derivedTenantJobPodName := cavObjNew.Spec.CAPApplicationInstance + "-provider-" + workload.Name + "-q4m9c-c4d9c"
		if len(derivedTenantJobPodName) > maxNameLength {
			return validateResource{
				allowed: false,
				message: fmt.Sprintf(
					...
					InvalidationMessage,
					v1alpha1.CAPApplicationVersionKind,
					derivedTenantJobPodName,
					len(derivedTenantJobPodName),
					maxNameLength,
					workload.Name,
				),
			}
