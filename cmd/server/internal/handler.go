...
const (
	...
	MetadataUpdateTimestamp  = "sme.sap.com/subscription-updated"
)
...
func (s *SubscriptionHandler) updateSubscription(reqInfo *RequestInfo, subPro *v1alpha1.SubscriptionProvider, sub *v1alpha1.Subscription) (bool, error) {
	...
	sub.Annotations[MetadataUpdateTimestamp] = time.Now().String()
...
func (s *SubscriptionHandler) getDependencies(req *http.Request, subscriptionType subscriptionType) ([]byte, error) {
	...
	// Dependencies are precomputed by the controller and published to the SubscriptionProvider spec
	if subPro.Spec.Dependencies == "" {
		util.LogInfo("No subscription dependencies found", GetDependencies, subPro, nil)
		return nil, nil
	}

	util.LogInfo("Subscription dependencies resolved", GetDependencies, subPro, nil, "dependencies", subPro.Spec.Dependencies)

	return []byte(subPro.Spec.Dependencies), nil
}
