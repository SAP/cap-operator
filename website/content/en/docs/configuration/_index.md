...
- `MAX_CONCURRENT_RECONCILES_CAP_APPLICATION`: the number of concurrent reconciles for CAPApplication e.g. `1`.
- `MAX_CONCURRENT_RECONCILES_CAP_APPLICATION_VERSION`: the number of concurrent reconciles for CAPApplicationVersion e.g. `3`.
- `MAX_CONCURRENT_RECONCILES_CAP_TENANT`: the number of concurrent reconciles for CAPTenant e.g. `10`.
- `MAX_CONCURRENT_RECONCILES_CAP_TENANT_OPERATION`: the number of concurrent reconciles for CAPTenantOperation e.g. `10`.
- `MAX_CONCURRENT_RECONCILES_DOMAIN`: the number of concurrent reconciles for Domain e.g. `1`.
- `MAX_CONCURRENT_RECONCILES_CLUSTER_DOMAIN`: the number of concurrent reconciles for ClusterDomain e.g. `1`.