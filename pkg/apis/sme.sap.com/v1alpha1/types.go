...
type CAPApplicationVersionSpec struct {
	...
	// Configuration for the service to be exposed (relevant only for 'Service' type deployment workloads)
...
// ServiceExposure specifies the details of the VirtualService to be exposed for a service
...
type Route struct {
	...
	// A unique routing path match/prefix used to route requests to the workload (when omitted, "/" would be used)
